# MIGRATION TODO — SimplCommerce → .NET Aspire + Blazor

> Checklist chi tiết từng phase. Mỗi item phải tick xong và commit trước khi sang item tiếp theo.
> Format: `- [ ] TASK-ID | mô tả | output kỳ vọng`
...
- [ ] P0-01 | Tạo `MIGRATION_DECISIONS.md` ở root với template (Date / Context / Decision / Consequences)
- [ ] P0-02 | Tạo `MIGRATION_PROGRESS.md` ở root, copy toàn bộ checklist này vào
- [ ] P0-03 | Tạo `MIGRATION_INVENTORY.md` ở root (sẽ fill ở các bước dưới)
- [ ] P0-04 | Tạo thư mục `docs/migration/` chứa các tài liệu phụ

### 0.2 Inventory module
- [ ] P0-05 | Liệt kê tất cả project trong `src/Modules/`, ghi vào `MIGRATION_INVENTORY.md` bảng: Tên module | Path | Số file .cs | Có wwwroot/admin? | Có Views? | Có Areas? | Có Migrations?
- [ ] P0-06 | Đọc `SimplCommerce.sln`, đối chiếu module nào `IsBundledWithHost=true` (xem `module.json` mỗi module)
- [ ] P0-07 | Liệt kê dependency giữa các module (qua `<ProjectReference>` trong .csproj) → vẽ dependency graph dạng Mermaid trong `docs/migration/module-dependencies.md`

### 0.3 Inventory API & Controllers
- [ ] P0-08 | Grep tất cả file kế thừa `Controller` hoặc `ControllerBase`, ghi bảng: Module | Controller | Route | HTTP methods | Có `[Authorize]`?
- [ ] P0-09 | Phân loại thành 3 nhóm: Storefront API (gọi từ AngularJS public), Admin API (gọi từ AngularJS admin), MVC Controller (return View)
- [ ] P0-10 | Output: `docs/migration/api-inventory.md`

### 0.4 Inventory UI
- [ ] P0-11 | Liệt kê tất cả file `.html` trong `wwwroot/admin/` của mỗi module — đây là AngularJS templates cần viết lại sang Blazor
- [ ] P0-12 | Liệt kê tất cả file `.cshtml` trong `Views/` và `Areas/*/Views/` — đây là Razor View MVC cần viết lại sang Razor Component
- [ ] P0-13 | Đếm số màn admin (file html) theo module, ưu tiên hoá top-10 màn có nhiều sub-template nhất
- [ ] P0-14 | Output: `docs/migration/ui-inventory.md`

### 0.5 Inventory cross-cutting
- [ ] P0-15 | Liệt kê tất cả `INotification` và `INotificationHandler` (MediatR domain events): Event | Publisher module | Handler module(s)
- [ ] P0-16 | Liệt kê external integrations: Stripe, PayPal, MoMo, VNPay, SendGrid, file storage local. Ghi config keys hiện tại
- [ ] P0-17 | Liệt kê background jobs / `IHostedService` / scheduled tasks
- [ ] P0-18 | Liệt kê `appsettings.json` keys: connection strings, payment provider keys, email config, file paths

### 0.6 Setup môi trường .NET 9
- [ ] P0-19 | Update `global.json` lên `.NET 9.0.x` SDK mới nhất
- [ ] P0-20 | Verify `dotnet --info` ra .NET 9
- [ ] P0-21 | Cài Aspire workload: `dotnet workload install aspire`
- [ ] P0-22 | Verify `dotnet new list aspire` thấy templates

### 0.7 Snapshot baseline
- [ ] P0-23 | Build solution cũ: `dotnet build SimplCommerce.sln` — phải PASS
- [ ] P0-24 | Chạy WebHost cũ với SQL local, login admin, screenshot homepage + admin dashboard, lưu vào `docs/migration/baseline-screenshots/`
- [ ] P0-25 | Export schema database hiện tại: `dotnet ef migrations script` từ WebHost → `docs/migration/baseline-schema.sql`
- [ ] P0-26 | Note version chính xác mọi NuGet package quan trọng vào `docs/migration/baseline-versions.md`

### 0.8 Commit Phase 0
- [ ] P0-27 | `git add -A && git commit -m "chore(migration): phase 0 - discovery and inventory"`
- [ ] P0-28 | Push branch, tạo PR draft "Migration Phase 0", merge vào `aspire-migration/main`
- [ ] P0-29 | Báo cáo theo template trong prompt → DỪNG, chờ user OK
